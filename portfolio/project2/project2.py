...
def my_imfilter(image, fil):
  ...
  if(img.ndim == 3):            # checks if array is 3d or 2d
    red = GetRedPixels(img)      # if image is 3d, each channel is separated from original image 
    ...
    img_padded = np.pad(img, ((rowsToPad,rowsToPad),(colsToPad,colsToPad)), 'edge')     # pad each channel
    out_img = convolve(img, img_padded, fil)
  return out_img
