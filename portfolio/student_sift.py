...
def get_features(image, x, y, feature_width, scales=None):
    ...
    paddedImage = np.pad(image, ((pad_val,pad_val),(pad_val,pad_val)), 'constant', constant_values=0)

    for i in range(len(x)):
        featurePoint = [int(y[i]+pad_val), int(x[i]+pad_val)]
        window = paddedImage[
            featurePoint[0]-pad_val:featurePoint[0]+pad_val,
            featurePoint[1]-pad_val:featurePoint[1]+pad_val
        ]
        fv.append(window.flatten())
